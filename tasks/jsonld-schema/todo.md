...
- [x] **T1: `clampSchemaText` truncation helper + tests**
...
- [x] **T2: cap PDP JSON-LD `name` (150) and `description` (5000)**
...
- [x] **T3: verify-deploy assertions for JSON-LD field lengths**
...
### Checkpoint 1
- [x] Gates: `pnpm typecheck && pnpm test && pnpm lint && pnpm build` — all zero-fail
- [x] ⚠ Deploy authorization → push origin + upstream → poll live
- [x] `node scripts/verify-deploy.mjs` fully green (48 existing + new)
- [ ] ⚠ USER PENDING: re-run GSC URL inspection on the audit URL → both length warnings gone

## Phase 2 — P1

- [x] **T4: LocationPicker accessible name (a11y + agentic)**
...
- [x] **T5: sku plumbing (inert until feed regen)** ⚠ prod DB migration
...
- [x] **T6: ⚠ Browser-assisted AWIN session — Create-a-Feed regeneration + Toolbox review** (coordinates with parity-audit P2-1)
...
### Checkpoint 2
- [x] Gates + `check-i18n.mjs` green
- [x] ⚠ Deploy authorization → push → verify-deploy green (incl. button-name assertion)
- [x] Post-regen ingest fill-rate report posted: sku 819/819, mpn 819/819, ean 567/819, model_number 0 (feed empty; `model` heuristic covers pipe-suffix names)

## Phase 3 — P2 (independent; parallelizable)

- [x] **T7: contrast fixes on PDP price/CTA column**
...
- [x] **T8: conservative trailing-model-code extraction → `model`**
...
- [x] **T9: BreadcrumbList JSON-LD emits crumbs 1–2 only**
...
- [x] **T10: drop `priceSpecification` sub-node** (APPROVED 2026-07-15)
...
- [x] Gates green; ⚠ deploy authorization → push → full verify-deploy green (53 assertions, incl. same-day GDPR-hotfix cycle — see audit closeout.md)
- [ ] ⚠ USER PENDING: re-run Rich Results Test + GSC inspection + Lighthouse on the audit URL
- [x] Results archived into `audit/2026-07-15_jsonld-schema/closeout.md`
