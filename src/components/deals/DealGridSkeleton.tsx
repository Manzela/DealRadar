 ...
 */
export function DealGridSkeleton({ count = 8 }: Readonly<{ count?: number }>) {
  const keys = Array.from({ length: count }, (_, i) => `deal-skeleton-${i}`);
  return (
    <div aria-busy="true" aria-live="polite" className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
      ...
      {keys.map((key) => (
        <div key={key} className="flex flex-col gap-3 rounded-xl border border-zinc-100 p-4">
