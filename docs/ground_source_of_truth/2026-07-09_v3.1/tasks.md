...
> **Exit criterion (gate):** prod `curl` of a remediated route = **200**; `grep dealradar.eu` in prod output = **0**; `pg_constraint` shows the FK + CHECKs; a refresh with **no live creds writes 0 rows**; _(v3.1)_ `schema.sql` defines `record_price_history` **exactly once**; `git diff main..HEAD -- scripts/ingest-awin.cjs` is **empty-or-superset**; `git status` shows **no untracked files** under `docs/`, `tasks/`, or `audit/` _(partially satisfied at `b53bb3e`: `docs/specs/` + `tasks/` are committed; the two `ground_source_of_truth` suites, `docs/recovered/`, and `audit/2026-07-09_consolidation/` remain untracked)_.

- [ ] T-INF-12: Commit the at-risk ground-source-of-truth, recovered, and spec/plan doc trees to git — the **first task executed** (data-loss prevention; RSK-13 ALREADY OCCURRED on 2026-07-09).
...
- [ ] T-DB-0: ⛔ **P0** — delete the duplicate `record_price_history()` from `supabase/schema.sql`, unifying to **ONE** day-aware, currency-carrying, NULL-safe definition — BEFORE any schema apply or any merge to main.
  ...
  - Verify: grep count of `function record_price_history` definitions in `supabase/schema.sql` = 1; staging apply → price-changing upsert → +1 day row; `select prosrc from pg_proc where proname='record_price_history'` contains `ON CONFLICT (product_id, day)` — **probe prod BEFORE and AFTER** (three-state: the prod DB's current function body and `price_history` shape are UNKNOWN).
...
- [ ] T-INF-1: Apply the remediation + on-disk schema migration to the **prod** Supabase DB (A-07) via the allow-listed idempotent runner.
  ...
  - Type: DETERMINISTIC (CI + `psql ON_ERROR_STOP=1`; additive & idempotent, so no ASK-FIRST, but it is a prod DB mutation — run via `db-migrate`, not by hand). **⛔ v3.1 precondition: T-DB-0 first — as merged, `schema.sql` is the P0 carrier, and `db-migrate.yml:11-17` auto-applies it on any push to main touching it: the break would self-deploy. Do NOT merge `schema.sql` to main until the duplicate block (`:249-270`) is deleted.**
...
- [ ] T-INF-2: Deploy the `feat/tier1-remediation-2026-06-28` build (branch HEAD — `b53bb3e` at authoring; `155cf06` is the code tree) to prod Netlify — close the merged≠deployed gap.
  ...
  - Type: DETERMINISTIC (Netlify build+deploy of the branch). **⛔ v3.1 precondition (A-02): deploying HEAD as-is is BLOCKED by the two `155cf06` P0s — T-DB-0 (schema dup) and T-ING-6 (ingest revert) must land first, else the deploy ships the price-clobber/no-merchant_url regression and a merge to main self-applies the broken schema.**
